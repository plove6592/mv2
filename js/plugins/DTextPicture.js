 ...
 * @author トリアコンタン
 * @version 1.1.2 2015/11/07 描画文字列に半角スペースが含まれていた場合も問題なく実行できるよう修正
 ...
 *  2 : イベントコマンド「ピクチャの表示」（Show Picture）で「画像」（Image）を
 *      未選択（None）に指定
 ...
 *
 * 対応制御文字一覧（イベントコマンド「文章の表示」（Show Text）と同一です）
 * \V[n]
 * \N[n]
 * \P[n]
 * \G
...
(function () {
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        if (command && command.toUpperCase() === "D_TEXT") {
            if (isNaN(args[args.length - 1])) args.push(0);
            var text = "";
    ...
    Sprite_Picture.prototype.makeDynamicBitmap = function() {
        ...
        var bitmap = new Bitmap(window.textWidth(textState.text) * 2, window.calcTextHeight(textState, false));
